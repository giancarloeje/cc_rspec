...
describe 'Update IDP mapping roles', type: :request do
  ...
  it 'should update idp mapping of cc roles (xml) when auth token used belongs to a root user' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.xml?auth_token=#{auth_token(@root_user)}"
    ...
    p response.body
    assert response.body.include?('<'+@role1.name+'>QA_Special, Maintenance</'+@role1.name+'>')
    assert response.body.include?('<'+@role2.name+'>QA_Special, Maintenance</'+@role2.name+'>')
  end

  it 'should update idp roles of cc roles (json) when auth token used belongs to a root user' do
    @root_user = FactoryBot.create(:user, authentication_token: Devise.friendly_token, :company => @company, :email => "root@domain.com")
    @root_user.is_root = true
    @role1.saml_idp_mapping = "QA"
    @role2.saml_idp_mapping = "QA"
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.json?auth_token=#{auth_token(@root_user)}"
    ...
    post url, :roles => @role1.name + ", "+@role2.name
    p response.body
    assert response.body.include?('{"roles":{"'+@role1.name+'":"QA_Special, Maintenance","'+@role2.name+'":"QA_Special, Maintenance"}}')
  end

  it 'should update idp roles of cc roles (xml) when auth token used belongs to an admin user' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.xml?auth_token=#{auth_token(@admin_user)}"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
    assert response.body.include?('<status>2 role/s updated</status>')
  ...
  it 'should update idp roles of cc roles (json) when auth token used belongs to an admin user' do
    @admin_user = FactoryBot.create(:user, authentication_token: Devise.friendly_token, :company => @company, :email => "admin@domain.com")
    @admin_user.is_root = false
    @admin_user.is_admin = true
    @admin_user.save!
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.json?auth_token=#{auth_token(@admin_user)}"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
  ...
  it 'should return an error message (xml) when auth token used belongs to a common user' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.xml?auth_token=#{auth_token(@common_user)}"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
  ...
  it 'should return an error message (json) when auth token used belongs to a common user' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.json?auth_token=#{auth_token(@common_user)}"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
  ...
  it 'should return an error message (xml) when auth token used is invalid' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.xml?auth_token=invalid"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
  ...
  it 'should return an error message (json) when auth token used is invalid' do
    ...
    @role1.save!
    @role2.save!
    url = "/#{@company.key}/#{@application.key}/roles/update_idp_mapping.json?auth_token=invalid"
    post url, :roles => @role1.name + ", "+@role2.name, :idp_roles => %w[QA_Special Maintenance]
    p response.body
