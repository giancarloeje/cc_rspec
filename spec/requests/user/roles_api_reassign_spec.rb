...
describe 'Reassign User API', type: :request do
  ...
  it 'should reassign roles to user when authentication token used belongs to a root user (JSON)' do
    ...
    @root_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=#{auth_token(@root_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('"status":"2 user/s updated"')
  end

  it 'should reassign roles to user when authentication token used belongs to a root user (XML)' do
    @root_user = FactoryBot.create(:user, authentication_token: Devise.friendly_token, :company => @company, :email => "root@gdslink.com")
    @root_user.is_root = true
    @root_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.xml?auth_token=#{auth_token(@root_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('<status>2 user/s updated</status>')
  end

  it 'should reassign roles to user when authentication token used belongs to an admin user (JSON)' do
    ...
    @admin_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=#{auth_token(@admin_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('"status":"2 user/s updated"')
  end

  it 'should reassign roles to user when authentication token used belongs to an admin user (XML)' do
    @admin_user = FactoryBot.create(:user, authentication_token: Devise.friendly_token, :company => @company, :email => "admin@gdslink.com")
    @admin_user.is_root = false
    @admin_user.is_admin = true
    @admin_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.xml?auth_token=#{auth_token(@admin_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('<status>2 user/s updated</status>')
  end

  it 'should not be able to reassign roles to user when authentication token used belongs to a common user (JSON)' do
    ...
    @common_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=#{auth_token(@common_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
  ...
  it 'should not be able to reassign roles to user when authentication token used is invalid (JSON)' do
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=invalid_token"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('Invalid auth-token')
  end

  it 'should not be able to reassign roles to user when authentication token used is invalid (XML)' do
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=invalid_token"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('Invalid auth-token')
  ...
  it 'should show an error message when users specified in request do not exist (JSON)' do
    ...
    @admin_user.save!
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=#{auth_token(@admin_user)}"
    post url, :roles => [@role1.name], :users => %w[no_user1@domain.com no_user2@domain.com]
    p response.body
    assert response.body.include?('no_user1@domain.com, no_user2@domain.com user/s not found in '+@company.key+' company')
  end
  ...
  it 'should show an error message when common user\'s role has no Role API permission (XML)' do
    ...
    url = "/#{@company.key}/#{@application.key}/roles/reassign.xml?auth_token=#{auth_token(@common_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
    p response.body
    assert response.body.include?('You are not allowed to use this API')
  end
  ...
  it 'should reassign roles to user when common user\'s role has Role API permission (XML)' do
  ...
  it 'should reassign roles to user when common user\'s role has Role API permission (JSON)' do
    @role_api = FactoryBot.create :role, API_role: true, application: @application
    @common_user = FactoryBot.create(:user, authentication_token: Devise.friendly_token, :company => @company, :email => "admin@gdslink.com", :is_root => false, :is_admin => false, :role_ids => [@role_api.id])
    url = "/#{@company.key}/#{@application.key}/roles/reassign.json?auth_token=#{auth_token(@common_user)}"
    post url, :roles => [@role2.name, @role3.name], :users => [@user1.email, @user2.email]
